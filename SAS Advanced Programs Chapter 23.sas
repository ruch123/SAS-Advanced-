...
libname chap16 'C:\UC Berkley\Summer 2016\Chapter 16';
libname chap23 'C:\UC Berkley\Summer 2016\Chapter 23';
...
** Just want white vs non-white differentiated;
...
**          Then you will see each of the individual results;
...
data heart_attack2(keep=count disdate rename=(disdate=quarter));
 ...
 proc freq data=heart_attack2;
  table quarter / missing;
   title 'here is count';
   title2 'Results summarized by quarter';
  ...
  proc sort data=heart_attack;
   ...
   proc means data=heart_attack;
    var age;
        ...
        ** Notice with the class statement you get the total
           which you do not get with the by statement;
...
** The Sortby is a feature or variable that is part of the dataset;
** It tells you have the dataset is sorted;
...
proc print data=sortby;
 title 'here is sortby';
 title2 'Notice that the Sortedby Describes how the varible is sorted';
 title3 '0 means no sort, 1 sorted small to highest first sort 2 means second sort';
...
**         This way the sorting can hopefully done all in memory and will;
**         Therefore be faster than if you have to bring it into your hardrive;
...
proc print data=heart_attack_age;
 var id age;
title 'Note Heart_attack is sorted by age';
...
You can also use the TAGSORT option to sort a large data set. The TAGSORT option
stores only the BY variables and the observation numbers in temporary files. The BY
variables and the observation numbers are called tags. At the completion of the sorting
process, PROC SORT uses the tags to retrieve records from the input data set in sorted
order.
...
proc sort data=ae0 out=third nodupkey noequals;
 ...
 title2 'Notice that we Do Not pick the first observations';
